...
impl<'a> Processor<'a> {
    pub fn new(blueprint: &'a Blueprint) -> Processor<'a> {
    ...
    pub fn dfs(&mut self, mut state: State, max: &mut State) {
...
impl State {
    ...
    pub fn get_resource(&self, resource: &Stone) -> i32 {
...
impl Blueprint {
    pub fn parse_input(input: &str) -> Vec<Self> {
        let (_, blueprints) =
